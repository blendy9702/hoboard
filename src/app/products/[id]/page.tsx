...
import { keyboards, getKeyboardById, LAYOUT_LABELS } from "@/data/keyboards"
...
export default function ProductDetailPage({
  ...
  const related = keyboards.filter(
    (kb) => kb.id !== keyboard.id && kb.layout === keyboard.layout
  ).slice(0, 3).concat(
    keyboards.filter(
      (kb) => kb.id !== keyboard.id && kb.layout !== keyboard.layout
    ).slice(0, Math.max(0, 3 - keyboards.filter(kb => kb.id !== keyboard.id && kb.layout === keyboard.layout).length))
  ).slice(0, 3)
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
                  ...
                  <button
                    onClick={() => setQuantity(Math.min(keyboard.stock, quantity + 1))}
                    className="w-10 h-12 flex items-center justify-center text-gray-500 hover:bg-gray-50 transition-colors text-lg"
                  >
                    +
                  </button>
              ...
              {keyboard.stock <= 5 && (
                <p className="mt-3 text-[12px] text-red-400 font-medium">
                  ⚠ 잔여 재고 {keyboard.stock}개 — 품절 전에 서두르세요
                </p>
              )}
